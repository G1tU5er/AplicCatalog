    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                default:
                    ModelState.AddModelError("", "Incercare nereusita.");
                    return View(model);
            }
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public ActionResult Register(RegisterViewModel model, params string[] selectedRoles)
        {
            var context = new ApplicationDbContext();
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    { throw new Exception("Trebuie sa selectati un rol!"); }
                }
                catch (Exception ex)
                {
                    ModelState.AddModelError("","Trebuie sa selectati un rol!");
                    ViewBag.RoleId = new SelectList(context.Roles.Where(x => x.Name != "Admin"), "Name", "Name");
                    return View();
                }
                var result = UserManager.Create(user, model.Password);

                if (selectedRoles != null && (selectedRoles.Contains("Profesor")))
                {
                    Profesor ut = new Profesor();
                        try
                        {
                            ut.user = user;
                            context.Entry(ut.user).State = EntityState.Unchanged;
                            context.Profesori.Add(ut);
                            context.SaveChanges();
                        }
